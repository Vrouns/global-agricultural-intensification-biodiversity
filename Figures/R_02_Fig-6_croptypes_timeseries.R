...
full_data <- read.csv("output/biodiversity_impact_assessment_2000-2019.csv")
data_croptypes <- full_data|>filter(!is.na(crop_type))
...
# ggsave("F4_croptypes_2panel.pdf",
#        plot = plots,
#        path = outp_path,
#        width = 8.16,  
#        height = 5.05,
...
plot_change_reg_ab <- ggplot(region_data, aes(x = year,
  ...
  scale_fill_manual(values= col_reg, labels = c("Brazil","Mexico","Other South \nAmerica", 
                                                "Indonesia","China","Other Asia \nand Pacific", 
                                                "DR Congo", "Tanzania", "Other Africa", 
                                                 "Other North America","Europe")) +
  labs(x = NULL, y = NULL, fill = NULL) +
  scale_y_continuous(limits = c(-0.23, 0.87)) +
...
outp_path = "./output/figures"
# for 1-panel-figure: 
ggsave("Ts_biodiv_change_croptypes.pdf",
       plot = plots_stacked,
       path = outp_path,
       width = 180,  
       height = 120,
