...
area_needed_inc <- min(cum_area_inc[cum_impact_inc >= impact_threshold])

area_needed_inc

### now with area 
area_threshold <- 0.10   # 10%
...
impact_top10_inc <- max(cum_impact_inc[cum_area_inc <= area_threshold])
...
# Per Intensity ---------------------------------------------------------------
###########
data_int <- rast("output/figures/LUH2_GCB2025/data_change_int.tif")
data_int <- data_int*1e06
#names(data_int) <- c("high","med","low","abandoned")
#plot(data_int$low)
...
ggsave(plot = int_plot, filename= "output/figures/LUH2_GCB2025/F03_Lorenz_Total_int_inc-dec.pdf",
       width = 8.0,
       height = 3.5,
       units = "in",
       device = "pdf")
...
sums <- 0
for(i in 1:nlyr(data_int)){
  sums[i] <- (sum(values(data_int[[i]]), na.rm = TRUE))
}
sum(sums)
##
# Hotspot map -------------------------------------------------------------
