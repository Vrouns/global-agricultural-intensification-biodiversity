...
intensity_folder <- ("./output/area_intensity/LUH2_GCB2025") # where intensity data is stored 
bia_path <- "./output/biodiversity_impact_assessment/LUH2_GCB2025"
...
croptype_df <- croptype_df %>% 
  ...
  mutate(intensity = recode(intensity, "light" = 1, "med" = 2, "high" = 3),
         intensity = factor(intensity, levels = c(1,2,3)))
...
lu_area_files_path <- list.files(paste0(intensity_folder), 
                                   pattern = "^pasture.*.csv$",full.names = T)
lu_area_list <- list()
...
write.csv(total_change, paste0("./output/lu_int_bia_area_LUH2_GCB2025_timeseries_2000-2019.csv"),
          row.names = F)

write_xlsx(total_change,"./output/lu_int_bia_area_LUH2_CROPGRID_timeseries_2000-2019.xlsx")
...
###
total_change <- read.csv("output/lu_int_bia_area_LUH2_GCB2025_timeseries_2000-2019.csv")
croptypes_group <- read.csv("./data/05_crop_types/CG/FAOSTAT/FAOSTAT_codes/crop_groups.csv",
                                                        header = T)

data_change_2019 <- total_change |>
  filter(year == 2019) |>
...
# Top 6 countries are: 
# "Brazil", "Democratic Republic of the Congo", "Indonesia", "India",
# "Peru", "Tanzania"
# --> exclude India, cause here main reason is land abandonment
...
total_change_plot_read<- total_change|>
  select("country","year","LU_type","intensity","crop_type","crop_group","country_group","SOV_A3",
         ...
         "area_ha","area_change")
write.csv(total_change_plot_read, "output/biodiversity_impact_assessment_2000-2019_LUH2_GCB2025.csv", row.names=F)
write_xlsx(data, "output/biodiversity_impact_assessment_2000-2019_LUH2_GCB2025.xlsx")

########################
# compare new vs old
LUH2_org_assessment <- read.csv("output/biodiversity_impact_assessment_2000-2019.csv")
LUH2_GCB_assessment <- read.csv("output/biodiversity_impact_assessment_2000-2019_LUH2_GCB2025.csv")

LUH2_org_2019 <- LUH2_org_assessment |>
  filter(year == 2019)

LUH2_GCB_2019 <- LUH2_GCB_assessment|>
  filter(year == 2019)

sum(LUH2_org_2019$impact_change, na.rm=T)
sum(LUH2_GCB_2019$impact_change, na.rm=T)

ggplot(LUH2_org_2019, aes(x = year, y = impact, fill = LU_type))+
