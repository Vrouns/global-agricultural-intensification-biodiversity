...
os.chdir("H:/02_Projekte/03_Intensification-fragmentation-CFs/Scripts/")
...
for t, year in enumerate(years):
    for lu_name, lu_idx in land_uses.items():
        data_dir_int = Path("../data/03_intensity/LUH2_GCB2025")
...
ds["area"].attrs["description"] = (
    ...
    "land use area for cropland and pasture is based on LUH2 GCB2025 (Chini, L., & Hurtt, G. (2025). Land-Use Harmonization 2 for Global Carbon Budget 2025 (LUH2-GCB2025) [Data set]. Zenodo. https://doi.org/10.5281/zenodo.15557904)"
)
...
ds.to_netcdf(
    "../output/zenodo/land_use_intensity_area_LUH2GCB.nc",
...
for t, year in enumerate(years):
    for lu_name, lu_idx in land_uses.items():
        data_dir_int = Path("../output/biodiversity_impact_assessment/LUH2_GCB2025/")
...
# store with compression
ds.to_netcdf(
    "../output/zenodo/land_use_intensity_biodiversity_impact_LUH2GCB.nc",
